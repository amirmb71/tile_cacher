...
def sql_db(x,y,z):
    ...
    name = '{}-{}-{}'.format(x, y, z)
    tiles_path = "/home/tomcat/image/{}".format(z)
    if not os.path.exists(tiles_path ):
        os.mkdir(tiles_path )


    ### mysql connetions
    cnx = mysql.connector.connect(user='chista', password='Chista@2020',host='127.0.0.1',database='tile_cache')
    cursor = cnx.cursor()
    cnx2 = mysql.connector.connect(user='chista', password='Chista@2020',host='127.0.0.1',database='tile_cache')
    cursor2 = cnx2.cursor()
    cnx3 = mysql.connector.connect(user='chista', password='Chista@2020',host='127.0.0.1',database='tile_cache')
    cursor3 = cnx3.cursor()
    cnx4 = mysql.connector.connect(user='chista', password='Chista@2020',host='127.0.0.1',database='tile_cache')
    cursor4= cnx4.cursor()
    ...
    if count[0][0] == 0:
        ...
        if int(c.getinfo(c.RESPONSE_CODE)) == 200:
            ...
            insert = ('INSERT INTO goole_tiles (name, modif_date, req_number ,resp_time,statuse) VALUES ("{}", "{}" , "{}" , "{}" , "{}" )'.format(name , modif_date , req_number ,resp_time ,statuse))
        ...
        req = ('SELECT req_number from google_tiles WHERE name = "{}" '.format(name))
        cursor3.execute(req)
        req_number = cursor3.fetchall()
        req_number = req_number[0][0] +1
        update = ('UPDATE google_tiles SET req_number = "{}" WHERE name = "{}"'.format(req_number , name))
